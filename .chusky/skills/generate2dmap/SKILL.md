...
name: generate2dmap
description: >
  Generate production-oriented 2D game maps with `CHUCK_GENERATE_IMAGE (prompt-only)`: RPG/top-down maps,
...
**Chusky runtime boundary:** `CHUCK_GENERATE_IMAGE` is prompt-only and accepts
`destination: "daytona"` or `"both"` when a generated map must be used in the
workspace. It returns a verified path for later Daytona work. It does not
accept source references; use Daytona only with a real uploaded or generated
file, then use `CHUCK_ARTIFACT` after map files pass validation.

## App-builder / Chusky environment

| Item | Value |
| --- | --- |
| Skill dir / scripts | `.chusky/skills/generate2dmap/`, run as `python3 .chusky/skills/generate2dmap/scripts/<script>.py …` |
...
- `CHUCK_GENERATE_IMAGE (prompt-only)` is the default art source for base maps, parallax plates,
...
- **A reference handoff is a file path, not a sentence.** For deterministic
  Daytona work, use a verified file path and inspect it before processing. The
  native image generator cannot currently consume a source path; do not claim
  that it can. Name the concrete features to preserve: camera
