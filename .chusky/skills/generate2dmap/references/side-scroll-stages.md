...
2. Hand the background to `Daytona deterministic image processing` as a real reference: pass its sandbox `file_path`. Also `read_file` it so you can see it; do not rely on a path string or prompt text as the reference.
