...
Pass the guide PNG's sandbox path to `Daytona deterministic image processing`. Tell the model to use it only for invisible slot count, spacing, centering, and safe padding. The output must not copy guide boxes, safe-area rectangles, center marks, labels, borders, or guide background.
