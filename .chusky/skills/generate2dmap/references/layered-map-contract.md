...
2. Hand the base to built-in `Daytona deterministic image processing` as a real reference: pass its sandbox `file_path` to `Daytona deterministic image processing`. Also call `read_file` so you can see it. Do not expect a filesystem path in the prompt, or the image merely being visible in conversation, to work as the visual reference.
