...
- A deployed app is provisioned a real database when it ships `migrations/*.sql`
  ...
  `"deploy": {"database": true}` in `.chusky/app-env.json` — see the `neon` skill.
...
- The app runs its **own** Better Auth at `/api/auth/*` and federates to the
  shared Chusky auth broker for **Google** and **X**. The only other supported
...
`.chusky/app-env.json` (`{"VITE_AUTH_ENABLED": "false"}`) is the switch, read by
...
signed-in Chusky viewers get the app session automatically from `x-chusky-identity`
(see the `auth` skill — `references/chusky-identity.md`); the broker federation
...
SDK → public connectors host (`connectors.chusky.me`, **auth required**) → gate.
...
skill** (`.chusky/skills/app-data/SKILL.md`) and follow it exactly. Do not invent
