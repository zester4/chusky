...
name: og
description: >
  Share-link previews and app identity for apps on *.chusky.me: the injector-owned
...
A deployed app (`https://{name}.chusky.me`) unfurls with a 1200×630 card; every app (preview included) shows a
favicon in the tab. **Share-card `<meta>` tags are not authored in `__root.tsx`** — the injector
(`scripts/chusky-pwa-shared.mjs`) overwrites `og:*` and `twitter:card` on every HTML response. Identity data is
...
keep the default `og.chusky.me` placeholder: no `public/og.jpg`. Its URL, the `"color"` knob and the rename
...
While the pass keeps `/workspace/.chusky/og-pending` fresh, brand checks say nothing about the card: in flight
...
that marker, stages files under `/workspace/.chusky/` — never inside `public/`, which `vite build` copies
...
that SVG, never `CHUCK_GENERATE_IMAGE (prompt-only)`**: it must stay crisp at 16px. Writing `site.json` for a game?
...
title move together. Without `CHUCK_GENERATE_IMAGE (prompt-only)` or the native image generation, keep the `og.chusky.me` card; never
