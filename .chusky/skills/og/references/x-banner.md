...
**Custom `public/x-banner.jpg` — games only.** Use `CHUCK_GENERATE_IMAGE (prompt-only)`
(or `Daytona deterministic image processing`) to generate a custom banner **only when the
app is a game**. Non-games omit the file — the injector then emits no
`x:game:image`. Do not paste `og.chusky.me/v1/banner.png` (or any
`x:game:image`) into `__root.tsx`; the injector strips those tags.
Generate the art at 50:11 (`aspect_ratio: "50:11"` on
`CHUCK_GENERATE_IMAGE (prompt-only)` — that ratio is on the Imagine schema; do **not**
...
ffmpeg -y -i banner-raw.jpg \
  ...
  -q:v 4 /workspace/.chusky/x-banner.jpg.tmp
node scripts/write-atomic.mjs /workspace/.chusky/x-banner.jpg.tmp public/x-banner.jpg
```
