...
`touch /workspace/.chusky/og-pending` before generating and again before each
...
`rm -f /workspace/.chusky/og-pending` on every exit path, success or not.
...
to a staged path under `/workspace/.chusky/` — never inside `public/`, which
...
`og.chusky.me` card (favicon, PWA icons, and title only — SKILL.md § "Decide:
