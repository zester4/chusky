...
description: >
  ...
  "per-user".
metadata:
  short-description: "Auth via the Chusky broker (Google, X) or local email/password — no other methods supported"
...
`/api/auth/*`, federating to the shared **Chusky auth broker** (`auth.chusky.me`)
...
not add entries to `CHUSKY_PROVIDERS`. Method detail and the email/password switch
...
buttons for them. Connector / app-data apps: ONLY gate "Continue with Chusky",
no Google/X buttons**: `references/chusky-identity.md`.

**Sign-in is OFF by default** — the template ships `.chusky/app-env.json` with
...
1. **Flag:** delete the `VITE_AUTH_ENABLED` key from `.chusky/app-env.json` and
...
- **Sign in / out:** `signIn(providerId)` and `signOut()` from
  `@/lib/auth/client`; `CHUSKY_PROVIDERS` renders the buttons. The popup,
