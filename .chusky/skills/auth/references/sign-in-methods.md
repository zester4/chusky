...
- **Google** and **X** — federated through the Chusky broker (pre-wired here). The
  broker federates these two upstreams and nothing else, so do **not** add entries
  to `CHUSKY_PROVIDERS` beyond them (the broker rejects an unknown `idp`).
- **Email + password** — this app's OWN Better Auth, persisted in your database
  ...
  **Do not** add `emailAndPassword` as a plugin entry (that is a syntax/type
  ...
  `*.chusky-sandbox.com` and local loopback on port 8080 (`localhost` /
...
- **`__Host-` cookies + `trustedOrigins`**: a sibling `*.chusky.me` app can't toss a
  `Domain=.chusky.me` cookie, and Better Auth rejects cross-origin `/api/auth`
  calls.
