...
| File | Use it for |
|---|---|
| `client.ts` | Browser client. `signIn(providerId)`, `signOut()`, `authEnabled`, `CHUSKY_PROVIDERS`. |
...
| `providers.ts` | `CHUSKY_PROVIDERS` — the fixed broker upstream list (Google and X only; don't add others). |
...
- **Live preview** (`*.chusky-sandbox.com`): the app is an embedded iframe, so
...
`src/lib/auth/preview.ts`, derives the `*.chusky-sandbox.com` origin per-request,
mints a process-stable session secret, and persists sessions in embedded
PGLite. Deployed apps get `CHUSKY_AUTH_*` / `BETTER_AUTH_*` / `DATABASE_URL`
...
| Var | Where | Purpose |
|---|---|---|
| `VITE_AUTH_ENABLED` | client | `"false"` in the shipped `.chusky/app-env.json` (dev user); drop the key to turn sign-in ON. Only client-visible auth flag |
...
| `CHUSKY_AUTH_ISSUER` | server | the shared broker (defaults to `https://auth.chusky.me`) |
| `CHUSKY_AUTH_CLIENT_ID` / `CHUSKY_AUTH_CLIENT_SECRET` | server | per-app client (falls back to the preview client) |
