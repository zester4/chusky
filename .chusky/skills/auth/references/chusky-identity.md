# Sign in with Chusky (deployed apps — zero clicks)

Behind the edge gate, every proxied request from a signed-in Chusky viewer
carries an unforgeable `x-chusky-identity` JWT (EdDSA, minted per request; the
...
`useSession` / `useCurrentUser` simply return the Chusky user.
...
concept**. Never render "Re-auth with Chusky", "Sign in again", "Refresh
session", or a standing "Continue with Chusky" button: sign-in UI may appear
...
it from `x-chusky-identity`, an instant sign-back-in loop. `<UserButton />`
...
| File | Role |
|---|---|
| `gate-identity.server.ts` | Verifies the gate's `x-chusky-identity` viewer JWT (EdDSA vs the gate JWKS; fail-closed). Server-only. |
...
| Var | Scope | Meaning |
|---|---|---|
| `CHUSKY_PROJECT_ID` | server | deployed apps: enables "Sign in with Chusky" (`x-chusky-identity` audience check `app:<project_id>`) |
| `CHUSKY_GATE_ORIGIN` | server | gate public origin override (JWKS + issuer pin); unset → preview mode (no `CHUSKY_PROJECT_ID`) defaults to the in-VM proxy `http://127.0.0.1:6014` (audience `preview`), deployed mode derives it from the inbound host |

Deployed behavior: gate-authenticated viewers are signed in automatically from
`x-chusky-identity`; the deployer also injects a per-app broker client +
...
Chusky" via the gate sign-in — the zero-click `x-chusky-identity` session above, or
...
apps without connector data. Still no new `CHUSKY_PROVIDERS` entries, still never
edit `src/lib/auth/`.
