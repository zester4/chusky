...
- raw sprite art must come from built-in `CHUCK_GENERATE_IMAGE (prompt-only)`, not Three.js, Canvas, SVG, HTML/CSS drawing, PIL shape drawing, procedural geometry, placeholder primitives, or code-rendered screenshots
...
- Pass the reference to built-in `Daytona deterministic image processing` as a sandbox `file_path`, and `read_file` it so you can see it too. Do not assume a path string in the prompt is a visual input.
...
When using a layout guide, pass the guide PNG's sandbox path to `Daytona deterministic image processing`. Use it only to understand the rows, columns, equal invisible frame slots, centering, spacing, and safe padding. Do not reproduce the guide: no visible boxes, no safe-area rectangles, no center marks, no labels, no borders, no guide background.
