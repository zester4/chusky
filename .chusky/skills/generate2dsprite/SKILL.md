...
metadata:
  short-description: "2D sprite sheets: CHUCK_GENERATE_IMAGE (prompt-only) + magenta chroma postprocess"
...
**Chusky runtime boundary:** `CHUCK_GENERATE_IMAGE` is prompt-only, but accepts
`destination: "daytona"` or `"both"` and returns a verified workspace path for
postprocessing. Use `telegram` only when no workspace handoff is needed. This
skill can postprocess only a real source file from a user upload or verified
Daytona result. Do not invent a path, and do not use a provider API or source-
aware generation helper that is not in the native tool catalog.

## App-builder / Chusky environment

| Item | Value |
| --- | --- |
| Skill dir | `.chusky/skills/generate2dsprite/` |
| Scripts | `python3 .chusky/skills/generate2dsprite/scripts/<script>.py …` |
| Image tools | `CHUCK_GENERATE_IMAGE` with `{ prompt }` for new images; source-aware editing is not exposed |
| Inspect images | `read_file` on the PNG path (not Codex view_image) |
| Generated image path | Not returned by the native tool; use a verified upload or Daytona path before processing |
...
Related skills: **`imagine`** (native image tool usage), **`game-asset-core`** (+
...
via `CHUCK_GENERATE_VIDEO (prompt-only; no source handoff)`), **`building-games`** (game loop / integration).
...
- Use built-in `CHUCK_GENERATE_IMAGE` for a new raw image, then continue only if
  a real file is available for the processor.
- Do not create raw sprite art with Three.js, Canvas, SVG, HTML/CSS drawing, PIL shape drawing, procedural geometry, placeholder primitives, or code-rendered screenshots. Runtime code may display finished generated assets, and scripts may make layout guides or postprocess generated images, but requested sprite art must originate from built-in `CHUCK_GENERATE_IMAGE (prompt-only)`.
- When the user provides a visual reference, use it for deterministic Daytona
  processing when possible. The current native image tool does not consume a
  source path for generative editing; do not claim that it does.
...
- Wire a real user-uploaded or verified Daytona reference into deterministic
  processing. The native generator does not consume reference paths, and its
  result is not automatically a local file. Also inspect local references
  before processing so you can verify them.
...
- Do not ask `CHUCK_GENERATE_IMAGE (prompt-only)` to generate unrelated action rows in one raw sheet, such as `row 1 idle, row 2 run, row 3 shoot, row 4 jump`, for a controllable hero or main character.
- Do not ask `CHUCK_GENERATE_IMAGE (prompt-only)` to generate raw single-row action strips such as `1x4 idle`, `1x4 run`, `1x4 shoot`, or `1x4 jump` for a controllable hero, character, creature, NPC, enemy, summon, or animated prop.
...
If a layout guide is useful, generate one before calling built-in `CHUCK_GENERATE_IMAGE (prompt-only)`:

```bash
python3 .chusky/skills/generate2dsprite/scripts/make_layout_guide.py \
  --rows <rows> \
  --cols <cols> \
  --cell-width 384 \
  --cell-height 384 \
...
Then pass the guide PNG's sandbox path to `Daytona deterministic image processing` — a guide that is only "visible in conversation" never reaches the image model. Also `read_file` it so you can see the geometry. Tell `Daytona deterministic image processing` to use it only for invisible slot count, spacing, centering, and safe padding. The output must not reproduce guide boxes, safe-area rectangles, center marks, labels, borders, or guide background.
...
Use built-in `CHUCK_GENERATE_IMAGE (prompt-only)`.
...
- keep the verified source file path
- copy that file into the working output folder as `raw-sheet.png` (or similar)
- keep the original source image in place
- to run a further deterministic edit on a **postprocessed** PNG, pass that
  PNG's verified Daytona path to the processing step
...
python3 .chusky/skills/generate2dsprite/scripts/generate2dsprite.py process \
