...
- Keep one verified base image and make deterministic variants in Daytona when
  possible. The native `CHUCK_GENERATE_IMAGE` tool is prompt-only and cannot
  consume the base as a generative source; do not claim that it can. Preserve
  the exact face, colors, proportions, outfit, scale, and background when
  changing only <X>.
