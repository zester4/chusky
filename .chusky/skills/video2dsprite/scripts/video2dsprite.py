#!/usr/bin/env python3
"""Postprocess verified local Chusky video clips into dense 2D sprites.
...
Native Chusky video generation is a separate asynchronous prompt-only workflow
and is not a local input to this processor.
...
def write_readme(out_dir: Path, meta: dict) -> None:
    lines = [
        "Video2dsprite output (Chusky Build pipeline)",
        "==========================================",
        "base/           base still on #FF00FF",
        "video/          verified local source clip",
        "frames-raw/     decoded frames",
        "frames-clean/   chroma-keyed RGBA frames",
        "sprite/         sampled normalized sprites + strips/grids/GIFs",
        "pipeline-meta.json",
        "",
        "This folder was produced by the deterministic Chusky verified-video processor.",
        "Native video generation is asynchronous and does not provide a local input here;",
        "the processor can still re-sample existing cleaned frames",
...
def cmd_process(args: argparse.Namespace) -> int:
    ...
    meta = {
        "skill": "video2dsprite",
        "platform": "Chusky verified local-video postprocessing",
        "name": args.name,
