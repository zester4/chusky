...
```text
1. source video        verified user upload or Daytona file (magenta when needed)
2. video               existing local MP4 (native generation is a separate async path)
...
- Non-Chusky agent without `CHUCK_GENERATE_VIDEO (prompt-only; no source handoff)`
