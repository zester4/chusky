...
If matching a project sprite, use a verified existing frame and deterministic
Daytona compositing when the art already exists. The native image generator is
prompt-only and cannot consume that frame as a source.

## Source video shot

When the user supplies or imports a clip, it should contain one short
present-tense shot (1–2 sentences of intended motion). Constraints:
