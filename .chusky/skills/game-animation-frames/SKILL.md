...
| Denser locomotion from video | **`video2dsprite`** — verified local video on **`#FF00FF`** → skill scripts (ffmpeg + chroma) |
...
Open `.chusky/skills/video2dsprite/SKILL.md` or `.chusky/skills/generate2dsprite/SKILL.md`
...
2. **Animate.** For a standalone generated clip, call `CHUCK_GENERATE_VIDEO`
   with a prompt and wait for normal channel delivery. It cannot consume the
   base still or hand a local file to Daytona. For this sprite pipeline, use a
   verified local video and keep the shot simple: one subject, one motion,
   static camera, and solid magenta when chroma processing is required.
...
Fall back to keyframe-by-keyframe `CHUCK_GENERATE_IMAGE (prompt-only)` (still on `#FF00FF` when
