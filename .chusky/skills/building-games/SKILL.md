...
**Controls / inverted A/D:** open **`.chusky/skills/controls/SKILL.md`** **before**
...
**Open `.chusky/skills/controls/SKILL.md` before implementing any WASD / steer /
...
- **2D game sprites / animation sheets** → run **`generate2dsprite`**
  (`.chusky/skills/generate2dsprite/SKILL.md`): solid **`#FF00FF`** magenta
  `CHUCK_GENERATE_IMAGE (prompt-only)` sheets + chroma postprocess scripts (magenta is required for the
...
- **2D maps / levels / prop packs** → open **`generate2dmap`**
  (`.chusky/skills/generate2dmap/SKILL.md`). Prefer foundation-only base + separate
...
- **Optional denser locomotion** → run **`video2dsprite`** (Chusky
  `CHUCK_GENERATE_VIDEO (prompt-only; no source handoff)` + sandbox scripts; magenta base). Prefer `generate2dsprite`
