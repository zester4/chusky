...
- Catalog presence ≠ user connected — use connection management when missing.
...
| Comms / channel | send via channel tools; image publish rules as configured |
| Voice / meetings / calls | meeting + phone tools under policy |
| Reminders / jobs | SET_REMINDER, SCHEDULE_JOB |
| Delegation | DELEGATE_SUBAGENT |

## Composio

1. Match domain via **composio-routing** skill when relevant.  
2. Confirm connection.  
3. Get schemas.  
4. Execute (single or multi).  
5. Multi-execute: pause if nested action is risky or unclassifiable.  
6. Image publish: follow staging rules; never pass raw bytes/URLs incorrectly.
