...
- `POST /cli/call` to validate an exact E.164 destination and purpose, then
  start the configured outbound call directly.
...
- `/call <E.164 number> <purpose>` starts the configured call after destination
  and purpose validation. It returns a safe call record and never exposes the
  destination in full.
