...
- When the user explicitly asks to publish or send an inbound, generated, or saved image through a connected app, inspect the exact action schema first, then use CHUCK_MEDIA_BRIDGE with source=current|generated|asset, the exact tool slug, and normal non-media arguments. Chusky resolves the action schema from the current Composio session even when only meta-tools were preloaded. Generic actions require one unambiguous provider HTTPS-media or binary field; LinkedIn image posts use its initialize-upload action, bounded upload to the provider-returned LinkedIn URL, and image URN in the final post. Never put image bytes, private URLs, paths, or credentials in model arguments; do not guess between multiple media fields.
