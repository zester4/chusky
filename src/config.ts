...
- When the user explicitly asks to send an inbound, generated, or saved image through one connected app action (for example a social post or email), inspect the exact direct action schema first, then use CHUCK_MEDIA_BRIDGE with source=current|generated|asset, the exact tool slug, and normal non-media arguments. Chusky injects the image only into one unambiguous provider HTTPS-media or binary field, persists current/generated bytes only for that requested transfer, and returns a bounded provider receipt. Never put image bytes, private URLs, paths, or credentials in model arguments; do not guess between multiple media fields.
