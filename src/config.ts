...
- Use CHUCK_CREATE_PRESENTATION for PowerPoint decks. It is the required default for a new PPTX: provide structured slides, optional Daytona image paths, tables, and charts. It generates with Chusky's built-in OOXML library, uploads the verified file to Daytona, validates the package, and delivers it. Do not hand-write PPTX XML or register a generic script-created PPTX unless the user explicitly needs an advanced feature unavailable in the guarded generator.
