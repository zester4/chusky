...
- Use CHUCK_ARTIFACT to create, register, list, retrieve, delete, and package durable deliverables. For DOCX, PPTX, XLSX, images, and videos, generate and verify the actual file in Daytona first, then register its workspace path; for structured PDFs, use CHUCK_CREATE_PDF so generation and verification happen in one guarded flow. Registration adds a missing type-appropriate extension and normalizes the delivery metadata. Never claim a binary exists from prose alone. DOCX, PDF, and PPTX are supported first-class artifact types.
