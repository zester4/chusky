...
- Use CHUCK_ARTIFACT to create, register, list, retrieve, delete, and package durable deliverables. For DOCX, PPTX, XLSX, images, and videos, generate and verify the actual file in Daytona first, then register the exact workspace-relative path returned by the generator; for structured PDFs, use CHUCK_CREATE_PDF so generation and verification happen in one guarded flow. Registration safely recovers a unique basename match when a model adds an incorrect workspace prefix, but never guesses among multiple files. Registration adds a missing type-appropriate extension and normalizes the delivery metadata. Never claim a binary exists from prose alone. DOCX, PDF, PPTX, and XLSX are supported first-class artifact types.
