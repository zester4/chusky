...
export async function deliverWebhook(hook: { id: string; url: string; secretCiphertext: string; contentType?: string; rawPayload?: boolean; authScheme?: string; authCredentialsCiphertext?: string; notificationTokenCiphertext?: string }, payload: unknown, fetchImpl: typeof fetch = fetch): Promise<{ delivered: boolean; status?: number; error?: string }> {
  const body = hook.rawPayload ? JSON.stringify(payload) : JSON.stringify({ id: `evt_${randomBytes(12).toString("base64url")}`, createdAt: new Date().toISOString(), data: payload });
  ...
  const headers: Record<string, string> = { "Content-Type": hook.contentType ?? "application/json", "User-Agent": "Chusky-Webhook/1.0", "X-Chusky-Webhook-Id": hook.id, "X-Chusky-Webhook-Timestamp": signed.timestamp, "X-Chusky-Webhook-Signature": signed.signature };
