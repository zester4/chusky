...
import { normalizeBrowserAlias, sessionHealth, type BrowserSessionHealth } from "./browserOps.js";
...
export async function beginVaultSetup(userId: number, request: VaultSetupRequest) {
  assertEnabled(); const service = normaliseVaultService(request.service); const origin = normaliseVaultOrigin(request.origin); const accountAlias = normalizeBrowserAlias(request.accountAlias);
  const result = await vaultBroker.beginSetup(account(userId), { service, origin, accountAlias, loginUrl: safeUrl(request.loginUrl || origin, origin, "loginUrl"), logoutUrl: request.logoutUrl ? safeUrl(request.logoutUrl, origin, "logoutUrl") : undefined, usernameFieldLabel: text(request.usernameFieldLabel || "Email or username", "usernameFieldLabel", 120), passwordFieldLabel: text(request.passwordFieldLabel || "Password", "passwordFieldLabel", 120), submitButtonLabel: text(request.submitButtonLabel || "Sign in", "submitButtonLabel", 120) });
