...
export async function loginWithVault(userId: number, service: string, browser: TrustedBrowserLogin, accountAlias = "default") {
  const workspaceId = browser.workspaceId ? await browser.workspaceId(userId) : "pending";
  const credential = await leaseVaultCredential(userId, service, workspaceId, accountAlias);
