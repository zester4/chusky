...
import { acquireUserLock, appendMessages, canSpend, cancelTask, checkRateLimit, claimApproval, createTask, createWebTelegramLinkCode, getApproval, getDaytonaWorkspace, getSession, getTask, getTelegramUserIdForWebAuth, isDurableStore, listChannelIdentities, listCliDevices, listFaceTimeCalls, listJobs, listOutbox, listReminders, listTasks, listHandoffRecords, getHandoffRecord, listVideoJobs, getVideoJob, updateOutbox, updateVideoJob, registerImageAsset, releaseUserLock, retryTask, saveHandoffRecord, saveSession, setApprovalStatus, setModel, setTaskWorkflowRunId, setVoiceReplies, type SdkProjectRecord, type SdkRunRecord, type SdkThreadRecord } from "./store.js";
...
async function sdkAgentOptions(body: RunBody): Promise<{ toolAllow?: string[]; toolDeny?: string[]; toolRequireApproval?: string[]; maxToolCalls?: number; maxCost?: number; instructions?: string }> {
  ...
  if (!skills.length) return { ...(allow ? { toolAllow: allow } : {}), ...(deny ? { toolDeny: deny } : {}), ...(requireApproval ? { toolRequireApproval: requireApproval } : {}), ...limits };
  const blocks: string[] = [];
...
export function registerSdkApi(app: Hono): void {
  ...
  app.post("/v1/threads/:threadId/runs", async (c) => {
    ...
    try { const result = await runAgent(owner.userId, resolved.message, thread.history, body.model ?? session.model, undefined, c.req.raw.signal, undefined, undefined, undefined, await sdkAgentOptions(body)); run.status = "completed"; run.output = result.text; run.events.push(event("run.completed")); thread.history.push({ role: "user", content: `${resolved.input || "Attached file(s)"}${resolved.attachments.length ? `\n[Attachments: ${resolved.attachments.map((file) => file.name).join(", ")}]` : ""}` }, { role: "assistant", content: result.text }); }
  ...
  app.post("/v1/threads/:threadId/runs/stream", async (c) => {
    ...
    const stream = new ReadableStream<Uint8Array>({ start: async (controller) => {
      ...
      try {
        const result = await runAgent(owner.userId, resolved.message, thread.history, body.model ?? session.model, undefined, abort.signal, (text) => { run.events.push(event("run.delta", text)); send({ type: "run.delta", runId: run.id, text }); }, undefined, undefined, await sdkAgentOptions(body));
  ...
  app.post("/v1/threads/:threadId/runs/:runId/resume", async (c) => {
    ...
    try { const resolved = await resolveRunInput(session, { input: prior.input, attachments: prior.attachments?.map((file) => file.id) }); const result = await runAgent(owner.userId, resolved.message, thread.history, run.model ?? session.model, undefined, c.req.raw.signal, undefined, undefined, undefined, await sdkAgentOptions({ budget: prior.budget, tools: prior.tools, skills: prior.skills })); run.status = "completed"; run.output = result.text; run.events.push(event("run.completed")); }
  ...
  app.get("/v1/approvals", async (c) => { const session = await getSession(sdkUser(c)!.userId); const data = session.approvals.filter((item) => item.status === "pending" && item.expiresAt > Date.now()).map((item) => ({ id: item.id, status: item.status, toolSlug: item.toolSlug, args: item.args, request: item.request, channelProvider: item.channelProvider, handoffId: item.handoffId, expiresAt: new Date(item.expiresAt).toISOString() })); return c.json({ data }); });
  ...
  app.post("/v1/workers/:id/cancel", async (c) => { const owner = sdkUser(c)!; const record = await getHandoffRecord(owner.userId, c.req.param("id")); if (!record) return apiError(c, 404, "not_found", "Worker delegation not found."); if (record.taskId) await cancelTask(owner.userId, record.taskId); const updated = await saveHandoffRecord(owner.userId, { ...record, status: "cancelled" }); return c.json(workerView(updated)); });
  ...
  app.get("/v1/deliveries", async (c) => { const data = (await listOutbox(undefined, 500)).filter((item) => item.userId === sdkUser(c)!.userId && !item.webhook).slice(0, 100).map((item) => ({ id: item.id, provider: item.provider, status: item.status, kind: item.kind, attempts: item.attempts, providerStatus: item.providerStatus, lastError: item.lastError, createdAt: new Date(item.createdAt).toISOString(), updatedAt: new Date(item.updatedAt).toISOString(), deliveredAt: item.deliveredAt ? new Date(item.deliveredAt).toISOString() : undefined })); return c.json({ data }); });
