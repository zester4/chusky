...
export function registerSdkApi(app: Hono): void {
  ...
  app.post("/v1/operator/outcomes/verify", async (c) => {
    const owner = sdkUser(c)!;
    const body = await c.req.json().catch(() => ({})) as Record<string, unknown>;
    const checks = Array.isArray(body.checks) ? body.checks.filter((item): item is OutcomeCheck => Boolean(item) && typeof item === "object" && typeof (item as Record<string, unknown>).id === "string" && typeof (item as Record<string, unknown>).description === "string").slice(0, 50) : [];
    if (!checks.length) return apiError(c, 400, "invalid_checks", "At least one outcome check is required.");
    const missionId = typeof body.missionId === "string" ? body.missionId : undefined;
    if (missionId && !await getMission(owner.userId, missionId)) return apiError(c, 404, "not_found", "Mission not found.");
    const verification = await executeOutcomeVerification({ ownerId: owner.userId, missionId, runId: typeof body.runId === "string" ? body.runId : undefined, checks, adapter: createUserOutcomeReadAdapter(owner.userId) });
    if (verification.status === "verified" && missionId) {
    ...
    return c.json(verification, verification.status === "verified" ? 200 : 409);
  });
