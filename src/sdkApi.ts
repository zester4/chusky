...
export function registerSdkApi(app: Hono): void {
  ...
  app.post("/v1/threads/:threadId/runs/:runId/resume", async (c) => {
    ...
    const run: SdkRunRecord = { id: `run_${randomUUID()}`, status: "running", input: prior.input, model: prior.model ?? session.model, attachments: prior.attachments, metadata: prior.metadata, budget: prior.budget, tools: prior.tools, skills: prior.skills, events: [event("run.started", "Resumed from a previous run")], createdAt: Date.now(), updatedAt: Date.now() }; thread.runs.push(run);
     try { const resolved = await resolveRunInput(session, { input: prior.input, attachments: prior.attachments?.map((file) => file.id) }); const result = await runAgent(owner.userId, resolved.message, thread.history, run.model ?? session.model, undefined, c.req.raw.signal, undefined, undefined, undefined, await sdkAgentOptions({ budget: prior.budget, tools: prior.tools, skills: prior.skills }, run.id, thread.id)); run.status = "completed"; run.output = result.text; run.events.push(event("run.completed")); }
  ...
  app.get("/v1/approvals", async (c) => { const data = (await listApprovals(sdkUser(c)!.userId, 100)).filter((item) => item.status === "pending" && item.expiresAt > Date.now()).map((item) => ({ id: item.id, status: item.status, toolSlug: item.toolSlug, args: item.args, request: item.request, channelProvider: item.channelProvider, handoffId: item.handoffId, expiresAt: new Date(item.expiresAt).toISOString() })); return c.json({ data }); });
