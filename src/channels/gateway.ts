  ...
  async processInbound(message: InboundMessage): Promise<ChannelProcessResult> {
    ...
    if (message.provider === "sendblue" && message.text?.trim().match(/^\/link[-_]?group$/i)) {
      await this.outbox.send({
        ...
        text: "To link this iMessage group, open Telegram and use /channel or /linkgroup (or /channel link sendblue-group), choose ‘Link an iMessage group’, then send /linkgroup <code> or /link-group <code> here from the linked owner account. WhatsApp group linking is not available through the WhatsApp Business API.",
