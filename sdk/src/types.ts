...
export interface AutonomyQueueItem { id: string; kind: AutonomyQueueKind; title: string; status: string; source: string; priority: number; nextAction?: string; nextCheckAt?: number; blockedReason?: string; updatedAt: number; }
export interface AutonomyProfile { id: string; userId: number; mode: "personal" | "business"; enabled: boolean; defaultAuthority: "observe" | "prepare" | "execute_reversible"; maxChecksPerDay: number; maxAutonomousActionsPerDay: number; notifyOn: "important" | "changes" | "all" | "silent"; allowedDomains: string[]; deniedDomains: string[]; quietHoursUtc?: { startMinute: number; endMinute: number }; createdAt: number; updatedAt: number; }
export interface AutonomySnapshot { userId: number; mode: "personal" | "business"; profile: AutonomyProfile; watches: Array<Record<string, unknown>>; queue: AutonomyQueueItem[]; counts: Record<string, number>; generatedAt: number; }
...
export interface ComposerStage extends ComposerStageInput { dependsOn: string[]; status: ComposerStageStatus; requiresApproval: boolean; retryLimit: number; taskId?: string; approvalId?: string; result?: string; }
export interface WorkflowComposerRecord { id: string; name: string; description?: string; stages: ComposerStage[]; status: "draft" | "queued" | "running" | "completed" | "failed" | "cancelled"; taskId?: string; workflowRunId?: string; createdAt: number; updatedAt: number; }
...
export interface ProviderMatrixEntry { surface: string; inboundText: boolean; inboundImage: boolean; outboundText: boolean; outboundImage: boolean; liveProof: "verified" | "configured_unverified" | "not_configured"; proofExpiresAt?: number; proofCorrelationId?: string; missing?: string; }
export interface OperatorReadiness { status: "ready" | "degraded" | "blocked"; generatedAt: number; durableStore: boolean; checks: Array<{ id: string; status: "passed" | "failed" | "warning"; detail: string }>; providerMatrix: ProviderMatrixEntry[]; blocking: string[]; warnings: string[]; }
export interface Compensation { id: string; ownerId: number; missionId?: string; missionStepId?: string; originalActionId: string; provider: string; objective: string; status: "pending" | "running" | "succeeded" | "failed" | "blocked" | "cancelled"; attempts: number; maxAttempts: number; idempotencyKey: string; createdAt: number; updatedAt: number; error?: string; resultSummary?: string; executionToolSlug?: string; externalReceiptId?: string; providerReceiptId?: string; verificationId?: string; }
