...
description: Start validated outbound calls and configure Chusky's live voices.
---
...
Call creation validates the E.164 destination and purpose, then starts the call
through the configured provider. It does not require a separate approval step.

```ts
const call = await chusky.calls.request({
