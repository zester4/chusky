...
The repository includes a guarded GitHub Actions workflow named **Chusky SDK Release**. It is started with **Run workflow** and accepts either an exact semantic version or a release type (`patch`, `minor`, or `major`). The local repository also exposes an exact-version CLI path so an operator can preflight or publish from a trusted release environment. The workflow:
